...
wake_up_radio Node_wur::send_wur() {
    ...
    int tx_p = this->transmission_power;
    int ch = this->channel;
    coordinates pos = this->location;

    wake_up_radio wur_signal;
...
// is being activated
void Node_wur::receive_wur(wake_up_radio interrupt) {
    wake_up_radio received_wur;

    received_wur.dst = interrupt.dst;
    received_wur.channel = interrupt.channel;
    received_wur.location = interrupt.location;

    double distance = devicesDistance(received_wur.location, interrupt.location);
