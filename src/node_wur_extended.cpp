...
Node_wur_extended::Node_wur_extended(int id, int x, int y, int z, int sf, int channel, int transmissionPower, double packetGenProb,
    ...
    this->states[8] = "RECEIVING_PACKET_AND_TRANSMITTING_WUR";
...
std::string Node_wur_extended::protocol() {
    ...
    if (this->current_state == states[1]) {
        ...
        if (this->buffer != nullptr) {
            ...
            if (receiver_timeout < 1) {
                ...
                // REMOVE REMAINED PACKETS IN BUFFER
                this->receiving_buffer.clear();
    ...
    if (this->current_state == states[5]) {
        ...
        // GO FOR RECEPTION
        if (this->wur_timer <= 1) {
            if (this->assigned_node < 0){
                ctrl_block_receive();
            }
            else {
                return ctrl_receive_packet_and_wur();
            }
            //KEEP GOING
        } else {
            this->wur_timer--;
    ...
    if (this->current_state == states[7]) {
        ...
        if (wur_timer_block_receive < 1) {
            ...
            wur_timer_block_receive--;// AFTER WUR GO FOR TRANSMISSION
            this->current_state = states[7];
            return this->current_state;
        }
    }

    // RECEIVING AND SENDING WUR
    if (this->current_state == states[8]){

        // SENDING WUR STILL IN PROGRESS
        if (this->wur_timer > 0) {
            this->wur_timer--;
