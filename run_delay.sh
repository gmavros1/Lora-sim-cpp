...
for r in {1..10}; do
  ...
  # Num nodes
  python3 ./network_init/place_nodes.py $((20 * r))
  ...
  for i in {1..3}; do
    ...
    python3 ./network_init/network_init.py "$rate" 500000 LoRaWAn 2 "adr"
    cd cmake-build-debug || return
    ./Lora_sim_cpp
    cd ..

    python3 ./network_init/network_init.py "$rate" 500000 LoRaWAn 3 "adr"
    cd cmake-build-debug || return
    ./Lora_sim_cpp
    cd ..
